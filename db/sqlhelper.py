...
class SqlHelper(object):
    ...
    def add(self, model):
        ...
        if isinstance(model, dict):
            ...
            values = (
                "'"
                + ",".join([str(v) for v in model.values()]).replace(",", "','")
                + "'"
            )
        ...
        # 添加的多个
            ...
            for item in model:
                ...
                values = (
                    "'"
                    + ",".join([str(v) for v in item.values()]).replace(",", "','")
                    + "'"
                )
        ...
        if conditions and isinstance(conditions, dict):
            ...
            for key, value in conditions.items():
                if key in allow_columns:
                    where += (
                        key + "=" + "'" + SqlHelper.escape_string(value) + "'" + " and "
                    )
    ...
    def get_valid_proxy(self, count=0, country=""):
        ...
        if count == 0 or not count:
            result = self.query(
                "select * from proxy_main where {} order by score desc".format(where)
            )
        else:
            result = self.query(
                "select * from proxy_main where {} order by score desc limit {}".format(
                    where, count
                )
            )
        return result
        ...
        for key, value in conditions.items():
            if key in allow_columns:
                where += (
                    ...
                    + "="
                    + "'"
                    + SqlHelper.escape_string(str(value))
                    + "'"
                    + " and "
                )
        where = where.strip("and ")
