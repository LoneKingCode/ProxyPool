...
class Parser(object):
    ...
    def get_proxy_data(url, urldata):
        ...
        try:
            if type == "xpath":
                ...
                LogUtil.error("网址配置错误，请填写正确的type,url:" + url)
            print("url:{} type:{} 获取数据数:{}".format(url, type, len(proxylist)))
            return proxylist
        except Exception as e:
            LogUtil.error("转换失败,URL:" + url + " 错误信息:" + str(e))
            return proxylist

    @staticmethod
    def xpath_parser(url, urldata):
        is_secret_cookie = (
            "cookie" in urldata.keys() and urldata["cookie"] == "secret_cookie"
        )
    ...
    def regular_parser(url, urldata):
        ...
        for proxy in proxy_data:
            ...
            proxylist.append(
                {"ip": ip, "port": int(port), "type": proxy_type, "protocol": protocol}
            )
        return proxylist
    ...
    def custom_parser(url, urldata):
        ...
        try:
            ...
            LogUtil.error("未找到指定的转换模块:%s 或转换出错:%s" % (modulename, str(e)))
        return proxylist
...
class ParserExtension(object):
    def goubanjia(urldata):
        ...
        for data in proxy_data:
            ...
            proxylist.append(
                {"ip": ip, "port": int(port), "type": type, "protocol": protocol}
            )
...
if __name__ == "__main__":
    a = Parser.get_proxy_data(
        "http://www.89ip.cn/tqdl.html?api=1&num=9999&port=&address=&isp=", UrlList[2]
    )
